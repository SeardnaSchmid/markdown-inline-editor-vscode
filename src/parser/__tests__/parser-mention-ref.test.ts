...
describe('MarkdownParser - Mentions and Issue References', () => {
  ...
  describe('@username mention', () => {
    it('should detect simple @username', () => {
      ...
      const mention = result.find((d) => d.type === 'mention' && d.slug === 'alice');
    ...
    it('should detect @user-name with hyphen', () => {
      ...
      const result = parser.extractDecorations(markdown);
      const mention = result.find((d) => d.type === 'mention' && d.slug === 'user-name');
      expect(mention).toBeDefined();
      expect(markdown.slice(mention!.startPos, mention!.endPos)).toBe('@user-name');
    });

    it('should detect @a1 alphanumeric', () => {
      const markdown = 'User @a1 reported';
      const result = parser.extractDecorations(markdown);
      const mention = result.find((d) => d.type === 'mention' && d.slug === 'a1');
      expect(mention).toBeDefined();
    });
    ...
    it('should not treat @user_name as mention (underscore not allowed)', () => {
      ...
      const result = parser.extractDecorations(markdown);
      const mention = result.find((d) => d.type === 'mention' && d.slug === 'user_name');
      expect(mention).toBeUndefined();
    });
  });
  ...
  describe('#123 issue reference', () => {
    it('should detect #123', () => {
      ...
      const result = parser.extractDecorations(markdown);
      const ref = result.find((d) => d.type === 'issueReference' && d.issueNumber === 123);
    ...
    it('should detect #1', () => {
      ...
      const result = parser.extractDecorations(markdown);
      const ref = result.find((d) => d.type === 'issueReference' && d.issueNumber === 1);
      expect(ref).toBeDefined();
    });
    ...
    it('should detect #456 in middle of text', () => {
      ...
      const result = parser.extractDecorations(markdown);
      const ref456 = result.find((d) => d.type === 'issueReference' && d.issueNumber === 456);
      const ref789 = result.find((d) => d.type === 'issueReference' && d.issueNumber === 789);
  ...
  describe('@org/team mention', () => {
    it('should detect @org/team', () => {
      ...
      const result = parser.extractDecorations(markdown);
      const mention = result.find((d) => d.type === 'mention' && d.slug === 'org/team');
      expect(mention).toBeDefined();
      expect(markdown.slice(mention!.startPos, mention!.endPos)).toBe('@org/team');
    });

    it('should detect @my-org/team-name', () => {
      const markdown = 'CC @my-org/team-name';
      const result = parser.extractDecorations(markdown);
      const mention = result.find((d) => d.type === 'mention' && d.slug === 'my-org/team-name');
      expect(mention).toBeDefined();
    });
  });
  ...
  describe('@user/repo#456 repo-scoped issue', () => {
    it('should detect @owner/repo#42 as single issueReference', () => {
      ...
      const ref = result.find(
        (d) => d.type === 'issueReference' && d.issueNumber === 42 && d.ownerRepo === 'owner/repo'
      );
      expect(ref).toBeDefined();
      expect(markdown.slice(ref!.startPos, ref!.endPos)).toBe('@owner/repo#42');
      // Should not have a separate mention for @owner/repo
      const mentionOnly = result.filter((d) => d.type === 'mention' && d.slug === 'owner/repo');
  ...
  describe('scopes for reveal-on-select', () => {
    it('should emit scopes for mention and issueReference', () => {
      ...
      const refDec = result.decorations.find((d) => d.type === 'issueReference');
      expect(mentionDec).toBeDefined();
      expect(refDec).toBeDefined();
      const scopeTexts = result.scopes.map((s) => markdown.slice(s.startPos, s.endPos));
    ...
    it('each mention and issueReference decoration should have a matching scope (reveal-on-select)', () => {
      const markdown = 'Contact @alice about #1 and @org/team or @owner/repo#99';
      const result = parser.extractDecorationsWithScopes(markdown);
  ...
  describe('large file', () => {
    it('should handle mention/ref scan on >10k line document without hanging', () => {
      ...
      const result = parser.extractDecorations(markdown);
      const mention = result.find((d) => d.type === 'mention' && d.slug === 'alice');
      const ref = result.find((d) => d.type === 'issueReference' && d.issueNumber === 42);
      expect(mention).toBeDefined();
      expect(ref).toBeDefined();
    });
  });
