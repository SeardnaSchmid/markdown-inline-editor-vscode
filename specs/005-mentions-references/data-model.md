...
The link provider computes `url` when in GitHub context from this metadata and `getGitHubContext`; the parser does not set `url`. When not in context, the link provider does not create DocumentLinks (styled text only). Extend `DecorationRange` or use existing optional fields to carry slug/number/ownerRepo as needed for resolution.
